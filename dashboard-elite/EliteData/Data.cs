...
namespace dashboard_elite.EliteData
{
    public class Data
    {
        ...
        private string npcSpeechBy(string from, string message)
        {
            string by;
        ...
        public Commander CommanderData = new Commander();
        public Target TargetData = new Target();
        public Location LocationData = new Location();
        public Dock DockData = new Dock();

        public Status StatusData = new Status();

        private bool HandleProfile(string pageName, bool state)
        {
            if (ActiveProfile != pageName && state)
            {
                if (_buttonCacheService.ButtonPageExists(pageName))
                {
                    ActiveButtonBlock = pageName;

                    ActiveProfile = ActiveButtonBlock;
        ...
        public void HandleStatusEvents(object sender, StatusFileEvent evt)
        {
            ...
            StatusData.HardpointsDeployed = (evt.Flags & StatusFlags.HardpointsDeployed) != 0 &&
                                            !StatusData.Supercruise && !StatusData.FsdJump;
            ...
            if (!HandleProfile("galaxymap", StatusData.GuiFocus == StatusGuiFocus.GalaxyMap))
            {
                if (!HandleProfile("systemmap", StatusData.GuiFocus == StatusGuiFocus.SystemMap))
                {
                    if (!HandleProfile("orrery", StatusData.GuiFocus == StatusGuiFocus.Orrery))
                    {
                        if (!HandleProfile("fssmode", StatusData.GuiFocus == StatusGuiFocus.FSSMode))
                        {
                            if (!HandleProfile("saamode", StatusData.GuiFocus == StatusGuiFocus.SAAMode))
                            {
                                if (!HandleProfile("infighter", StatusData.InFighter))
                                {
                                    if (!HandleProfile("srvturret", StatusData.SrvTurret))
                                    {
                                        if (!HandleProfile("insrv", StatusData.InSRV))
                                        {
                                            if (!HandleProfile("onfoot", StatusData.OnFoot))
                                            {
                                                if (!HandleProfile("analysismode", StatusData.HudInAnalysisMode))
                                                {
                                                    if (!HandleProfile("cargoscoop", StatusData.CargoScoopDeployed))
                                                    {
                                                        if (!HandleProfile("hardpoints", StatusData.HardpointsDeployed))
                                                        {
                                                            HandleProfile("default", true);
                                                        }
                                                    }
                                                }
                                            }
...
//#endif
        ...
        public void HandleBackPackEvent(object sender, BackPackEvent.BackPackEventArgs e)
        {
        ...
        public void HandleCargoEvent(object sender, CargoEvent.CargoEventArgs e)
        {
            ...
            {
                LimpetCount =
                    e.Inventory.Where(x => x.Name.ToLower().Contains("drones")).Sum(x => x.Count);
            }
        ...
        public void HandleEliteEvents(object sender, JournalEventArgs e)
        {
            ...
            switch (evt)
            {
                ...
                case "UnderAttack":
                    ...
                    {
                        UnderAttack = true;
                        LastUnderAttackEvent = DateTime.Now;
                    }
